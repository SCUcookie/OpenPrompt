...
- A background research pass (multi-agent) was launched to hunt for the blocked Strip R-CNN-S DIOR-R checkpoint, PKINet/PKINet-v2/LSKNet code+checkpoint status, the ConInfer `segearth_segmentor` fix, and dataset sources for the segmentation lane (WHU_Aerial, OVRSISBenchV2/OVRSIS95K, RSKT-Seg's OVSISBench), plus a literature-tracker refresh. See the dated follow-up note once that run completes for exact findings/URLs/commands; do not assume any specific checkpoint was found until that note is read.
