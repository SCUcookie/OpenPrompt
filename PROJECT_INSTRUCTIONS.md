...
- DIOR-R S4 pseudo-label purification is formally treated as **closed**: three closure attempts (short-pack, low-LR continuation, LR5e-6 micro-sweep) topped out at best mean `0.697272`/final mean `0.693988`, still below the S3 best-mean gate `0.6979` and the single-run gate `0.7000`. DIOR-R S3 scene/context adapter (best mean `0.6979`, best single replica rep0 epoch 8 `0.6992`, final mean `0.6859`) remains the primary GeoNexus-RSD DIOR-R result. Do not relaunch S4 pseudo-label training with the same recipe without a design change and a separate route decision.
- The TGRS manuscript `_local_archive_20260601_pull_backup/docs/TGRS/geonexus_tgrs.tex` was refreshed from its 2026-06-14 evidence state to 2026-07-09: added DIOR-R S3 (active, completed) and S4 (closed) to the abstract, contributions, Method (Context Modulation, Pseudo Label Purification), a new "DIOR-R Comparator Context" subsection with a comparator table/figure (`table_tgrs_sota_comparators`, `fig:tgrs_sota_comparators`), and updated Discussion/Conclusion. Fixed a stale DIOR-R S2 rounding bug carried over from the old script/paper (`0.6884/0.6853` corrected to the actual `0.6887/0.6856`, matching the 2026-06-14 status note above). Regenerated all TGRS figures/tables via a new `scripts/make_tgrs_result_assets_20260709.py` (extends the 2026-06-14 generator) and recompiled cleanly to a 9-page PDF with `pdflatex` (3 passes, no undefined references, no errors). The architecture figure `geonexus_tgrs_architecture.tex` was updated so Panel D shows the S3 context adapter as active (wired into the score-fusion output) and the S4 branch as tested-and-closed rather than purely "future."
- The DIOR-R comparator table/figure states plainly that this is comparator context, not a SOTA claim: only OrientedFormer Swin-T was independently re-run under our protocol (confirmed `0.6883` AP50, reproduced identically twice); Strip R-CNN-S, AOPG, and the OrientedFormer public-paper rows are unverified-protocol public numbers, and PKINet/PKINet-v2/LSKNet were left out of the comparator figure because no verified public DIOR-R AP50 was available for them at generation time.
- A background multi-agent research pass hunted for the blocked Strip R-CNN-S DIOR-R checkpoint, PKINet/PKINet-v2/LSKNet code+checkpoint status, the ConInfer `segearth_segmentor` fix, and dataset sources for the segmentation lane. **It hung for 4+ hours** because one sub-agent's `WebFetch` on a Microsoft OneDrive folder redirect (`1drv.ms/f/...`, for RSKT-Seg's OVSISBench dataset) never returned and there is no enforced timeout on that call; the task was killed manually via `TaskStop`. 21 of 22 sub-agent calls had already completed with independently-verified findings before the hang, so the run was not wasted — only the RSKT-Seg leg needed a manual, fetch-free write-up afterward. **Lesson for future sessions: do not `WebFetch` `1drv.ms` or other OneDrive/SharePoint folder-listing redirect URLs without a strict timeout wrapper; treat them as manual-browser-only sources.**
- Confirmed-blocked, no usable artifact exists (do not relitigate without new evidence):
  - **Strip R-CNN-S DIOR-R checkpoint**: independently re-verified twice. The README's DIOR-R download link (Google Drive id `1_c2aXANKHl0cIBb370LNIkCyDmQpA3_o`) is byte-identical to the ImageNet backbone-only file (page names it `stripnet_s.pth`, 159M) — the same ID is reused on 4 unrelated README rows (backbone/DOTA-v1.0-MoCAE/DIOR-R/HRSC2016), a copy-paste bug. Corroborated by two open, unanswered GitHub issues (#21, #20) reporting AP=0/low mAP from that exact download, an empty GitHub Releases list, and the newer `HVision-NKU/Strip-R-CNN` mirror having silently deleted the DIOR-R row rather than fixing it. No HF/Baidu/OneDrive alternative found.
  - **LSKNet DIOR-R checkpoint**: repo model zoo covers DOTA-v1.0/FAIR1M/HRSC2016/segmentation sets only; the only "dior" hit in the whole tree is an unrelated Strip R-CNN config with a 404 base file. No LSKNet DIOR-R artifact exists.
  - **PKINet / PKINet-v2 DIOR-R checkpoint**: both official repos (`PKINet/PKINet`) confirmed to have zero DIOR-R (or HRSC2016) configs/checkpoints anywhere (only DOTA-v1.0/v1.5 configs and OneDrive-hosted DOTA/v1.5 checkpoints). A DIOR-R result for PKINet would require training from scratch; only a third-party PKINet-v1 (not v2) DIOR-R config was found, linked from GitHub issue #18, unverified.
- Downloaded locally (small artifacts, under `New/artifacts/blocked_files_20260709/`): the missing ConInfer `segearth_segmentor.py` fix (from `likyoo/SegEarth-OV` at pinned commit `3e22a969b32c6d751bdbba64a88a0b670e630f55`, with provenance note — the shipped ConInfer configs use `type='ConInferSegmentation'`, so this file's class may need renaming/wrapping before use, not a drop-in); a full shallow clone of the official `ConInfer` repo (`coninfer_repo/`); a full shallow clone of `SegEarth-OV` (`segearth_ov_repo/`); and two shallow clones of the official `PKINet/PKINet` repo (`pkinet_code_checkpoint/`, `pkinet_v2_code_checkpoint/` — same repo, since PKINet-v2 configs live in the same GitHub org/repo as v1). These are plain local git checkouts, not committed to Git (see Repository Boundary rule); the user must copy/push them out-of-band if wanted on the server.
- Ready-to-run server-side download commands recorded (not fetched locally — all large): Pi-Seg checkpoints (`kkk2026/Pi-Seg_for_OVRSISBenchV1`/`V2` on Hugging Face, smallest file confirmed 835MB) and datasets (`kkk2026/OVRSIS95K` ~30GB essential / up to ~89GB with a redundant `train_parts/` folder to exclude, `kkk2026/OVRSISBenchtest` ~44GB, `kkk2026/OVRSISBenchV2o3` ~5GB), all public/ungated, via `huggingface-cli download`; WHU_Aerial for ConInfer (`http://gpcv.whu.edu.cn/data/...zip`, confirmed 5.06GiB via `curl -I`, plus a Kaggle mirror); PKINet-v2 OneDrive-hosted checkpoint weights (curl with `&download=1`, with an `onedrivedownloader` Python fallback noted since OneDrive sometimes serves an HTML interstitial to non-browser clients). RSKT-Seg's OVSISBench dataset and pretrained weights are Baidu-Netdisk-primary (password `USTC`) with a OneDrive mirror link that is suspiciously identical between the "dataset" and "weight" README entries (likely another copy-paste bug); Baidu requires a Chinese-network account/client and was not attempted.
- Literature tracker refreshed: `docs/literature/20260607_openrsd_related_recent_papers.md` now has a `2026-07-09 Refresh` section. Highest-priority new item: **RiO-DETR** (arXiv 2603.09411, 2026-03) reports DOTA-1.0/DIOR-R/FAIR-1M-2.0 results and states code will be released, but is not yet released and its exact DIOR-R number is not confirmed from the primary source (a search-engine summary cited 75.73 AP50, materially above GeoNexus S3's 69.79 — treat as unconfirmed, do not cite as fact). Added as a watch-only note in the TGRS manuscript's Discussion; do not add to the comparator table until code/checkpoint and a confirmed number exist.
- Segmentation lane (ConInfer/RSKT-Seg/Pi-Seg, `New/BRIEF_LIST.md`) stays **paused/secondary** behind the core DOTA2/DIOR-R GeoNexus-RSD route by default, per this file's own "keep the paper scope small" and "do not drift into a large new framework" rules. This is a default, not a final call — revisit explicitly if the user elevates its priority.
