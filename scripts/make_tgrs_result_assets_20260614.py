"""Generate paper-ready TGRS result assets for the 2026-06-14 evidence state.
...
def collect_data() -> dict:
    ...
    for rep in s2_dior["replicas"]:
        ...
        ap50s = [float(m["dota_AP50"]) for m in rep["metrics"]]
        dior_s2_runs.append(
            DiorS2Run(
    ...
    assert_round("DIOR-R S2 best mean", s2_dior_best_mean, 0.6884)
    assert_round("DIOR-R S2 final mean", s2_dior_final_mean, 0.6853)
    ...
    main_rows = [
        ...
        StageValue("DIOR-R", "DIOR_R_dota/test", "S2 hierarchy", "best-checkpoint mean mAP", s2_dior_best_mean, "3", rel(s2_dior_path), "best checkpoint mean"),
        StageValue("DIOR-R", "DIOR_R_dota/test", "S2 hierarchy", "final mean mAP", s2_dior_final_mean, "3", rel(s2_dior_path), "final checkpoint mean"),
    ]
    return {
...
def render_main_results(data: dict) -> None:
    labels = ["DOTA2\nS0", "DOTA2\nS1", "DOTA2\nS2 best", "DOTA2\nS2 final", "DIOR-R\nS0", "DIOR-R\nS1", "DIOR-R\nS2 best", "DIOR-R\nS2 final"]
    values = [row.value for row in data["main_rows"]]
    colors = [GRAY, BLUE, GREEN, ORANGE, GRAY, BLUE, GREEN, GREEN]
    ...
    ax.set_ylabel("mAP")
    ax.set_ylim(0.585, 0.700)
    ax.text(0.02, 0.96, "Formal evidence summary", transform=ax.transAxes, fontsize=8.6, fontweight="bold", va="top")
    ax.text(0.02, 0.875, "DOTA2 S2 separates best-checkpoint and final means; DIOR-R S2 is complete.", transform=ax.transAxes, fontsize=6.7, color=MUTED)
    style_axis(ax)
...
def render_dior_progression(data: dict) -> None:
    means = [data["s0_dior"], data["s1_dior_mean"], data["s2_dior_best_mean"], data["s2_dior_final_mean"]]
    labels = ["S0\nfinal", "S1\nfinal mean", "S2\nbest mean", "S2\nfinal mean"]
    fig, ax = plt.subplots(figsize=(3.45, 2.35))
    x = list(range(4))
    ax.bar(x, means, width=0.58, color=[GRAY, BLUE, GREEN, GREEN], alpha=0.9, edgecolor=INK, linewidth=0.35)
    ax.plot(x, means, color=INK, marker="o", markersize=3.5, linewidth=1.1)
    for xi, val in zip(x, means):
        ax.text(xi, val + 0.0011, f"{val:.4f}", ha="center", va="bottom", fontsize=6.5)
    for j, val in enumerate(data["s1_dior_finals"]):
        ax.scatter(1 - 0.055 + j * 0.11, val, s=18, facecolor="white", edgecolor=BLUE, linewidth=0.8, zorder=4)
    for j, run in enumerate(data["dior_s2_runs"]):
        offset = -0.09 + 0.09 * j
        ax.scatter(2 + offset, run.best_map, s=20, marker="D", facecolor="white", edgecolor=GREEN, linewidth=0.8, zorder=4)
        ax.scatter(3 + offset, run.final_map, s=20, marker="o", facecolor="white", edgecolor=GREEN, linewidth=0.8, zorder=4)
    best = max(data["dior_s2_runs"], key=lambda r: r.best_map)
    ax.annotate(
        f"{best.name} e{best.best_epoch}: {best.best_map:.4f}",
        xy=(2, best.best_map),
        xytext=(1.45, best.best_map + 0.0056),
        arrowprops={"arrowstyle": "->", "lw": 0.65, "color": GREEN},
        fontsize=6.2,
        color=INK,
    )
    ax.set_xticks(x, labels)
    ax.set_ylabel("DIOR-R mAP")
    ax.set_ylim(0.648, 0.696)
    ax.text(0.01, 0.98, "DIOR-R progression", transform=ax.transAxes, fontsize=8.3, fontweight="bold", va="top")
    style_axis(ax)
    fig.tight_layout(pad=0.2)
...
def render_result_synthesis(data: dict) -> None:
    ...
    dior_labels = ["S0", "S1\nmean", "S2\nbest", "S2\nfinal"]
    dior_values = [data["s0_dior"], data["s1_dior_mean"], data["s2_dior_best_mean"], data["s2_dior_final_mean"]]
    ...
    ax_note.text(
        0.18,
        0.67,
        "DIOR-R completed S0->S1->S2 progression; DOTA2 S2 final remains unstable.",
        transform=ax_note.transAxes,
        fontsize=6.7,
        color=INK,
        va="center",
    ...
    ax_note.text(
        0.18,
        0.34,
        "No pseudo-label, efficiency, or qualitative detection claims are made in this evidence state.",
        transform=ax_note.transAxes,
        fontsize=6.7,
        color=MUTED,
        va="center",
...
def write_tables(data: dict) -> None:
    ...
    (TAB_DIR / "table_tgrs_main_results.tex").write_text(
        latex_table(
            ...
            rows=main_rows,
            caption="Main GeoNexus-RSD evidence on DOTA-v2.0 ss_val and sanitized DIOR-R test. DOTA-v2.0 S2 reports best-checkpoint and final-checkpoint means separately; DIOR-R S2 is complete and no longer pending.",
            label="tab:tgrs_main_results",
            widths="@{}p{0.11\\textwidth}p{0.18\\textwidth}p{0.18\\textwidth}p{0.16\\textwidth}cp{0.03\\textwidth}p{0.23\\textwidth}@{}",
            starred=True,
    ...
    dior_rows = [
        ...
        ["S2 hierarchy", "best mean", data["s2_dior_best_mean"], "3", "20260614 DIOR-R S2 JSON"],
        ["S2 hierarchy", "final mean", data["s2_dior_final_mean"], "3", "20260614 DIOR-R S2 JSON"],
    ]
    ...
    (TAB_DIR / "table_tgrs_dior_r.tex").write_text(
        latex_table(
            ...
            rows=dior_rows,
            caption="Sanitized DIOR-R results on DIOR_R_dota/test using MMRotate DOTAMetric mAP at IoU 0.5. S1 is the mean of two final checkpoints; S2 is summarized by both best-checkpoint and final-checkpoint means across three replicas.",
    ...
    source_tex_rows.extend(
        [[f"DIOR-R S2 {r.name}", f"seed {r.seed}", "mAP", f"{r.best_map:.4f}/{r.final_map:.4f}", f"20260614 DIOR-R S2 JSON; scalar record {r.name}"] for r in data["dior_s2_runs"]]
    )
...
def write_fragments() -> None:
    fragments = {
        "fragment_fig_tgrs_main_results.tex": figure_fragment(
            ...
            "Formal GeoNexus-RSD result summary on DOTA-v2.0 ss_val and sanitized DIOR-R test. Bars report mAP; DOTA-v2.0 S2 separates the seven-run best-checkpoint mean from the final-checkpoint mean to avoid overclaiming unstable final behavior.",
        ...
        "fragment_fig_tgrs_dior_progression.tex": figure_fragment(
            ...
            "DIOR-R stage progression on sanitized DIOR_R_dota/test using MMRotate DOTAMetric mAP at IoU 0.5. Bars and the line show S0 final, S1 final mean over two replicas, S2 best-checkpoint mean over three replicas, and S2 final mean over three replicas; overlaid points show replica values, with the best single S2 replica at rep1 epoch 8.",
        ...
        "fragment_fig_tgrs_result_synthesis.tex": figure_fragment(
            ...
            "Result synthesis for the June 14 evidence state. DIOR-R uses sanitized DIOR_R_dota/test with MMRotate DOTAMetric mAP at IoU 0.5 and shows the completed S0 reference, S1 final mean over two replicas, and S2 best/final means over three replicas. DOTA-v2.0 uses DOTA2_1024_500/ss_val and reports S0, S1, and seven-run S2 loss-0 best/final means to make the best-versus-final instability explicit. No pseudo-label, efficiency, or qualitative detection claims are included.",
...
def write_readme(data: dict) -> None:
    text = f"""# TGRS Result Assets - 2026-06-14

Generated by `scripts/make_tgrs_result_assets_20260614.py`.
...
- DIOR-R: S0 `{data['s0_dior']:.4f}`, S1 final mean `{data['s1_dior_mean']:.4f}`, S2 best mean `{data['s2_dior_best_mean']:.4f}`, S2 final mean `{data['s2_dior_final_mean']:.4f}`.

DOTA v1.5 is intentionally excluded from the generated main result assets.
No pseudo-label, efficiency, or qualitative detection result is fabricated.
`geonexus_tgrs_result_synthesis` is the manuscript Figure 6 replacement and
summarizes the quantitative DIOR-R progression plus DOTA2 best/final boundary.
"""
    (TAB_DIR / "README_tgrs_result_assets_20260614.md").write_text(text, encoding="utf-8")
