...
The primary evaluation will use DOTA2 style rotated object detection. Images are split into manageable tiles, and annotations are converted to oriented boxes. DIOR-R is the required second dataset for cross-dataset validation, using the local \texttt{DIOR\_R\_dota/train\_val} and \texttt{DIOR\_R\_dota/test} splits. FAIR1M is reserved for a later fine-grained hierarchy study if DOTA2 and DIOR-R are stable. The method should be evaluated under standard supervised training and under a semi supervised extension where additional unlabeled or weakly labeled tiles are purified before use. The same prompt bank should be evaluated across datasets and prompt variants to measure vocabulary robustness.
...
\centering
\small
\caption{Comparison settings to run before submission. DOTA2 is the primary benchmark and DIOR-R is the required cross-dataset validation. The final manuscript should replace this protocol table with measured rotated mAP and class wise AP.}
\begin{tabular}{lll}
\toprule
...
The current repository implements the data loading, prompt bank, lightweight detector scaffold, DOTA style validation path, and an anchored box decoding repair path. The reduced tiled DOTA v1.0 and DOTA v1.5 scaffold runs, as well as the later DOTA v1.5 GeoNexus S1/S2/S3 diagnostics, are archive-only evidence for implementation behavior. They must not be presented as the paper's main performance evidence. The immediate experimental priority is to finish the DOTA2 closed-set baseline table, port the most defensible hierarchy-aware prompt scoring or hierarchy regularization module to the strongest stable DOTA2 detector, and then reproduce that minimal module on DIOR-R after a clean DIOR-R baseline smoke.

Current DOTA2 closed-set evidence includes RoI Transformer, Oriented R-CNN, S2ANet, and RTMDet-M baselines, with R3Det and RTMDet-L still under monitoring. GeoNexus S3/S4 should not be advanced until the DOTA2 S1/S2 module beats or clearly complements the strongest closed-set baseline. A standard oriented detector such as RoI Transformer or Oriented R-CNN should also be used for the first DIOR-R baseline before launching the matching GeoNexus variant.
