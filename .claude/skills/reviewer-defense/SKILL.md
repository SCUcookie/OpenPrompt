...
## GeoNexus-RSD project context

<!-- Installed 2026-07-22 from fcakyon/phd-skills (MIT, (c) 2026 Fatih
     Cagatay Akyon), content reviewed in full before installation; this
     section is the project-specific addendum. -->

Target venue: IEEE TGRS (journal review culture — thorough related work,
depth over breadth, writing quality weighted). Known vulnerabilities with
their current mitigation state, so questions are matched to real answers:
single detector family (R1 Oriented R-CNN campaign in progress — cite its
status honestly), single backbone (R2 Swin-T planned), no statistical
significance tests (we do report 2–7 seed replicas with best/final means —
frame that), DOTA-v2.0 numbers are validation-set not test-server (already
stated plainly in the paper — keep it), comparator rows partly cited
rather than reproduced (only OrientedFormer independently re-run; the
dagger convention and Strip R-CNN checkpoint-unavailability finding are
the defense), and the pseudo-label negative result (a strength if framed
as analysis — it already is). The strongest ablation story is the DIOR-R
per-class table: gains concentrate on scene-dependent/confusable classes
exactly as the method predicts.
