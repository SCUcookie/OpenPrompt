...
In this repository the number-tracing step has a fixed shape: every
manuscript number must trace to a JSON/note under `docs/experiments/`, and
tables are produced only by `scripts/make_tgrs_result_assets_<date>.py`
whose `assert_round` guards recompute means from those records — so the
numerical audit is "prose vs generator output vs evidence JSON", never
"prose vs memory". Known verified rounding split to respect, not flag:
OrientedFormer evaluator 68.83 (Table I/abstract) vs parsed per-class mean
68.84 (Table V, caption-noted). Component names are Baseline/TPC/HRR/SCA/
PLP; any S0–S4 stage label in prose is itself a HIGH finding. Combine this
skill with the `tgrs-manuscript` skill's banned-token and compile checks
for a full pre-submission audit.
